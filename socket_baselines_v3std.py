...
def BaselineMeanStd(data,chan):
    ...
    tempchunk = tempchunk['dataword'][tempchunk['packet_type']==1] # packet type 1 for v3, 0 for <v3
    ...
    if len(timestamps)>0 :
        ...
        for timestamp in timestamps.iloc[1:-1]:
            ...
            if abs(dt) != avgDT and abs(dt)%1001 != 0 :
                print("Time anomaly for chan {} dt={} and avgDT={} at index {} at timestamp {} after {}".
                      format(chan,dt,round(avgDT,0),index,timestamp,prevtimestamp))
...
zerochunk=datachunk[datachunk['dataword']==0]
zerochunk.to_csv("zerodata.csv",mode='w',header=True)
...
for chan in range(NumASICchannels): 
	...
	summaryFrame = pd.concat([summaryFrame,pd.DataFrame({'runtime':runtime,'Mean':mean[chan],'Std':sdev[chan],
	'Nent':nentries[chan],'ChanName':textchan,'Chan':chan,
	'ChipSN':ChipSN,'io_group':io_group,'io_channel':io_channel},index=[0])] )
